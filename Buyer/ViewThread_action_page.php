<?php
	...
	if(isset($_POST["view_thread"])){
		...
		$sql = "SELECT item_threads.item_thread_id, item_threads.book_title, item_threads.binding_type, item_threads.author, item_threads.genre, item_threads.price,
				item_threads.image_location, item_threads.company_id, item_threads.date_created, item_threads.is_closed, item_threads.date_closed, item_threads.is_deleted,
				users.is_active
				...
				where item_threads.item_thread_id = '".$_POST["item_thread_id"]."'
			";
		...
		if ($result2->num_rows == 1) {
			while($row2 = $result2->fetch_assoc()) {
				...
				foreach ($row2 as $key => $value) {
					...
					// Thread is closed if the seller marked it closed OR the company account was disabled.
					else if(strcmp($key,"is_closed") == 0){
						if(strcmp($value,"1") == 0 || strcmp($row2["is_active"],"0") == 0){
					...
					// Disregard item thread owner's user id
