<?php
	...
	if(isset($_POST["Messages"])){
		...
		// Select all distinct conversations with this user id
		$sql = "SELECT DISTINCT company_id from messages where user_id = '".$_POST["user_id"]."'
			";
		...
		if ($result2->num_rows > 0) {
			...
			while($row2 = $result2->fetch_assoc()) {
				...
				$sql = "SELECT * from messages join companies on messages.company_id = companies.company_id where messages.company_id = '".$row2["company_id"]."'
						and messages.user_id = '".$_POST["user_id"]."'
						order by datetime_created DESC
						LIMIT 1
					";
		...
		// 1. Check if that user email exists and it is a seller
		$sql = "SELECT companies.company_id, companies.email, companies.image_location, companies.name, users.is_active, users.user_type
			from companies join users on users.user_id = companies.user_id where companies.email = '".$_POST["email"]."' and user_type = 'seller'
			";
		...
		if ($result2->num_rows == 1) {
			while($row2 = $result2->fetch_assoc()) {
				...
				if(strcmp($row2["is_active"],"0") == 0){
					$myObj->message = "Unable to send message. That company account has been deactivated by the administrator.";
					$myObj->alert="danger";
				...
				$sql = "INSERT INTO messages (company_id,user_id,message,datetime_created,is_sent_by_buyer,is_admin_announcement)
							VALUES ('".$row2["company_id"]."','".$_POST["user_id"]."','".$_POST["message"]."','".$dt."','1','0')";
				...
				if ($conn->query($sql) === TRUE) {
					...
					$myObj->messages->company_id = $row2["company_id"];
					$myObj->messages->is_sent_by_buyer = "left"; // buyer sent the last message
