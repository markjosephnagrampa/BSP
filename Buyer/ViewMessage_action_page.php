<?php
	...
	if(isset($_POST["view_message"])){
		...
		$sql = "SELECT messages.message, messages.datetime_created, messages.is_sent_by_buyer, messages.is_admin_announcement,
					companies.name, companies.email, companies.image_location,
					users.is_active
				from messages 
				join companies on messages.company_id = companies.company_id 
				join users on companies.user_id = users.user_id
				where messages.user_id = '".$_POST["user_id"]."' and messages.company_id = '".$_POST["message_company_id"]."'
		...
		$sql = "INSERT INTO messages (company_id,user_id,message,datetime_created,is_sent_by_buyer,is_admin_announcement)
					VALUES ('".$_POST["message_company_id"]."','".$_POST["user_id"]."','".$_POST["message"]."','".$dt."','1','0')";
